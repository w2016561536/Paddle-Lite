...
#include "lite/operators/calib_conv2d_op.h"
#include <algorithm>
#include <vector>
...
bool CalibConv2dOpLite::CheckShape() const {
...
bool CalibConv2dOpLite::InferShapeImpl() const {
  const auto in_dims = param_.x->dims();
  const auto filter_dims = param_.filter->dims();
...
REGISTER_LITE_OP(calib_conv2d, paddle::lite::operators::CalibConv2dOpLite);
