...
void FpgaConvFuserWithBranch::BuildPattern() {
  ...
  auto* conv_filter = VarNode("ConvFilter");
  auto* conv_bias = VarNode("ConvBias");
  auto* conv_output = VarNode("ConvOutput");

  auto* depthwise_node = OpNode("depthwise_conv2d", "depthwise_conv2d");
  auto* depthwise_filter = VarNode("DepthwiseFilter");
  auto* depthwise_bias = VarNode("DepthwiseBias");
  auto* depthwise_output = VarNode("DepthwiseOutput")->assert_only_one_output();

  auto* hard_swish_node =
      OpNode("hard_swish", "hard_swish")->assert_node_satisfied(
          IsSupportedHardSwish);
  auto* hard_swish_output = VarNode("HardSwishOutput")->assert_only_one_output();
...
void FpgaConvFuserWithBranch::InsertNewNode(SSAGraph* graph,
                                            const key2nodes_t& matched) {
  std::cout << "insert new branch node for calib_conv2d" << std::endl;
